...
class WatchfaceDesigner {
    ...
    generateCode() {
        ...
        // Generate the draw function
        arduinoCode += `// Main draw function - call this in your Watchy drawWatchFace()\n`;
        arduinoCode += `void draw_${name}(GxEPD2_BW<WatchyDisplay, WatchyDisplay::HEIGHT> &display, tmElements_t currentTime) {\n\n`;

        // Draw background
        if (hasStaticContent) {
            arduinoCode += `  // Draw background bitmap\n`;
            arduinoCode += `  display.drawBitmap(0, 0, ${name}_background, 200, 200, GxEPD_BLACK);\n\n`;
        } else {
            arduinoCode += `  // Clear display with white\n`;
            arduinoCode += `  display.fillScreen(GxEPD_WHITE);\n\n`;
        }
    ...
    generateWidgetCode(widget) {
        ...
        switch (widget.type) {
            ...
            case 'weather-temp':
                return `  // ===== TEMPERATURE =====\n` +
                    `  // Position: (${x}, ${y}) Size: ${w}x${h}\n` +
                    `  // NOTE: You need to fetch weather data first!\n` +
                    `  display.setFont(&FreeSansBold${widget.fontSize}pt7b);\n` +
                    `  display.setTextColor(${color});\n` +
                    `  display.setCursor(${x}, ${Math.round(y + h * 0.8)});\n` +
            ...
            case 'weather-icon':
                return `  // ===== WEATHER ICON =====\n` +
                    `  // Position: (${x}, ${y}) Size: ${w}x${h}\n` +
                    `  // TODO: Draw weather icon based on weatherData.condition\n` +
                    `  // Example: display.drawBitmap(${x}, ${y}, sunny_icon, ${w}, ${h}, ${color});\n`;
            ...
            case 'weather-condition':
                return `  // ===== WEATHER CONDITION =====\n` +
                    ...
                    `  display.setTextColor(${color});\n` +
                    `  display.setCursor(${x}, ${Math.round(y + h * 0.8)});\n` +
                    `  display.print(weatherData.condition); // "Sunny", "Cloudy", etc.\n`;
            ...
            case 'weather-hi-lo':
                return `  // ===== HI/LO TEMPERATURE =====\n` +
                    ...
                    `  display.setCursor(${x}, ${Math.round(y + h * 0.8)});\n` +
                    `  display.print("H:");\n` +
                    `  display.print(weatherData.highTemp);\n` +
                    `  display.print("° L:");\n` +
                    `  display.print(weatherData.lowTemp);\n` +
                    `  display.print("°");\n`;
            ...
            case 'battery':
                return `  // ===== BATTERY INDICATOR =====\n` +
                    ...
                    `  int batteryPercent = map(batteryVoltage, 3.2, 4.2, 0, 100);\n` +
            ...
            case 'steps':
                return `  // ===== STEP COUNTER =====\n` +
                    ...
                    `  display.setTextColor(${color});\n` +
                    `  display.setCursor(${x}, ${Math.round(y + h * 0.8)});\n` +
                    `  uint32_t stepCount = sensor.getCounter(); // BMA423 step counter\n` +
                    `  if (stepCount >= 1000) {\n` +
                    `    display.print(stepCount / 1000.0, 1);\n` +
                    `    display.print("k");\n` +
                    `  } else {\n` +
